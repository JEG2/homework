...
def build_item(tables)
  ...
  case item
  when "Specific armor"
    build_specific_armor(tables)
  when "Specific shield"
    build_specific_shield(tables)
  when "Special ability and roll again"
    build_special_armor_ability(tables)
  when "Specific weapon"
    build_specific_weapon(tables)
  when "Special weapon ability and roll again"
    build_special_weapon_ability(tables, coin_flip)
  when "Wonder"
    build_rod(tables)
  end
  puts
...
def build_specific_armor(tables)
  puts choose_from_table(tables["Specific armor"])
end

def build_specific_shield(tables)
  puts choose_from_table(tables["Specific shield"])
end

def build_special_armor_ability(tables)
  armor = choose_from_table(tables["Armor and shields"])
  puts armor
  if armor == "+1 armor" || armor == "+2 armor" || armor == "+3 armor" || armor == "+4 armor" || armor == "+5 armor" || armor == "Specific armor"
    spec = "Special armor"
  else
    spec = "Special shield"
  end
...
def build_specific_weapon(tables)
  weapon = choose_from_table(tables["Specific weapon"])
  if weapon == "Slaying arrow"
    puts choose_from_table(minor_tables["Slaying arrow"])
  end
    puts weapon
end

def build_special_weapon_ability(tables, coin_flip)
  ...
  case weapon
  when "Bane"
    puts choose_from_table(minor_tables["Bane"])
...
def build_rod(tables)
    puts choose_from_table(med_tables["Wonder"])
end
