Tables = 
  {
    ...
    "Object" => {
      ...
      "Disturbing" => [
        ...
        "Broken hatch door with marks of fists in the metal", "Sealed orders", 
        "The Shroud of Turin", 
...
def category(dice, marker = "Relationship")
    puts "#{Tables[marker].keys[dice[0] - 1]} (#{dice[0]}) #{Tables[marker][Tables[marker].keys[dice[0] - 1]][dice[1] - 1]} (#{dice[1]})"
    puts
end

def category_relationship(dice)
  puts "Player: 1 and Player: 2"
  category(dice, "Relationship")
  dice = dice[2..15]
  puts "Player: 2 and Player: 3"
  category(dice, "Relationship")
  dice = dice[2..13]
  puts "Player: 3 and Player: 4"
  category(dice, "Relationship")
  dice = dice[2..11]
  puts "Player: 4 and Player: 1"
  category(dice, "Relationship")
end

def check_number_of_locations
  rand(1..2)
end
...
def run_program
  ...
  dice = dice[2..5]
  if check_number_of_locations == 2
    puts "Location:"
    category(dice, "Location")
  else
    puts "Object:"
    category(dice, "Object")
  end
  dice = dice[2..3]
  puts "Object:"
  category(dice, "Object")
